...
// Initial code (task) for the Callback Hell page
const initialCode = `// Callback Hell Task
// Create a nested callback function to print "Nested callback executed" after "Doing something..."
...
// Solution code for the Callback Hell task
const solutionCode = `// Callback Hell Solution
function doSomething(callback) {
  setTimeout(() => {
    console.log('Doing something...');
    callback(() => {
...
const CallbackHell = () => {
  ...
  useEffect(() => {
    const blockName = 'callback-hell'; // Explicitly set the block name for this component
  ...
  const handleCodeChange = (newCode) => {
    ...
    if (role === 'student') {
      ...
      socket.emit('codeChange', { code: newCode, blockName: 'callback-hell' }); // Emit code change event
    }
